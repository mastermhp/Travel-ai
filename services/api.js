...
class ApiService {
  ...
  // TRAVEL TASK MANAGEMENT METHODS

  async getTravelTasks() {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || `Failed to get tasks: ${response.status}`)
      }

      return data
    } catch (error) {
      console.error("Get travel tasks error:", error)
      throw error
    }
  }

  async createTravelTask(taskData) {
    try {
      const response = await fetch(`${this.baseUrl}/api/tasks`, {
        method: "POST",
        headers: this._getAuthHeaders(),
      ...
      console.error("Create travel task error:", error)
      throw error
    }
  }

  async updateTravelTask(taskId, updateData) {
    try {
      ...
      console.error("Update travel task error:", error)
      throw error
    }
  }

  async deleteTravelTask(taskId) {
    try {
      const response = await fetch(`${this.baseUrl}/api/tasks/${taskId}`, {
        method: "DELETE",
        headers: this._getAuthHeaders(),
      ...
      console.error("Delete travel task error:", error)
      throw error
    }
  }

  // TRAVEL TRIPS MANAGEMENT METHODS

  async getTravelTrips() {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || `Failed to get trips: ${response.status}`)
      }
  ...
  async createTravelTrip(tripData) {
    try {
      const response = await fetch(`${this.baseUrl}/api/trips`, {
        method: "POST",
        headers: this._getAuthHeaders(),
      ...
      if (!response.ok) {
        throw new Error(data.message || `Failed to create trip: ${response.status}`)
      }
  ...
  async getHotelRooms(hotelId, provider, params = {}) {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/api/hotels/rooms/${safeProvider}/${hotelId}?${queryParams.toString()}`,
        {
          method: "GET",
          headers: this._getAuthHeaders(),
        },
        ...
        if (match) {
          ...
          hotelData = parsedData.map((hotel) => {
            const locationQuery = encodeURIComponent(
              (hotel.location || destination).split(",")[0].trim().toLowerCase().replace(/\s+/g, "+"),
            )

            // Ensure images is always an array with at least one URL
            const images = Array.isArray(hotel.images) ? hotel.images : hotel.image ? [hotel.image] : []

            // Make sure each image is properly formatted
            const processedImages =
              images.length > 0 ? images.map((img) => (typeof img === "string" ? img : null)).filter(Boolean) : []
